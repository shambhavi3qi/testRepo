#Bootcamp Example 3: Firefox, Watir-webdriver, Project library, Excel data repository
...
# verify the account is gone
  ...
  def navigate_to_accounts_in_crm(browser)
    browser.link(:title, 'CRM Software').when_present.click
    sleep(2)
    browser.link(:id, 'tab_Accounts').when_present.click
    sleep(2)
  end
